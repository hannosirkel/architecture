...
- [ ] `architecture`: after the preceding PR merges, add the `initiatives/active/ai-portal.md` pointer in `universe/repositories.yaml`, update `initiatives/active/ai-portal/{state.yaml,resolved-plan.md}`, and make worktree validation use the current architecture root in `tooling/{universe_catalogue.py,test_catalogue.py}`; verify the regression test red-to-green, full unit suite, `ruff check .`, `tooling/universe validate`, and drift. The pointer cannot be in the first PR because the old validator checked the stale primary checkout.
